...
@section('content')
    ...
    <div class="card-header bg-light py-3 d-flex justify-content-between align-items-center">
        <h6 class="m-0 fw-bold text-dark">Laporan Penjualan</h6>
    </div>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-2 d-flex align-items-end gap-2">
                        <button type="submit" class="btn btn-primary w-100"><i class="fas fa-search me-1"></i>
                            Tampil</button>
                        <a href="{{ route('sales.index') }}" class="btn btn-light border" title="Reset"><i
    ...
    <div class="row mb-4 g-3">
        ...
        <div class="col-md-2">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body">
                    <div class="text-muted small mb-1">Pajak</div>
                    <h3 class="fw-bold text-dark mb-0">Rp {{ number_format($summary['tax'], 0, ',', '.') }}</h3>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        @forelse($sales as $s)
                            <tr>
                                <td class="ps-4">{{ \Carbon\Carbon::parse($s->date)->format('d/m/Y') }}</td>
                                <td class="fw-bold text-primary">{{ $s->code }}</td>
                                <td>{{ $s->customer_name }}</td>
                                <td class="text-end">Rp {{ number_format($s->subtotal, 0, ',', '.') }}</td>
                                <td class="text-end">Rp {{ number_format($s->discount, 0, ',', '.') }}</td>
                                <td class="text-end">Rp {{ number_format($s->tax, 0, ',', '.') }}</td>
                                <td class="text-end fw-bold pe-4">Rp {{ number_format($s->total, 0, ',', '.') }}</td>
                            </tr>
                        @empty
                            <tr>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-white py-3 border-bottom">
            <h6 class="m-0 fw-bold text-dark">Top 10 Produk Terjual</h6>
        </div>
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
